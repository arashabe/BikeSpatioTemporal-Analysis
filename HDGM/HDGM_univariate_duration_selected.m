%% Univariate HDGM for Duration (Selected Covariates)
...
%% Extract data
data.Y{1} = daily_data.bs_data.duration;  % Duration only
...
duration_model = stem_model(obj_stem_data, obj_stem_par);
duration_model.stem_data.log_transform;
duration_model.stem_data.standardize;
duration_model.set_initial_values(obj_stem_par);

obj_stem_EM_options = stem_EM_options();
obj_stem_EM_options.max_iterations = 200;
duration_model.EM_estimate(obj_stem_EM_options);

%% Validation and results
duration_model.set_varcov;
duration_model.set_logL;
RMSE_duration = sqrt(mean(duration_model.stem_validation_result{1,1}.cv_mse_s));

%% Save results
save('univariate_hdgm_duration_results.mat', 'duration_model');

%% Export results to text file
fileID = fopen('univariate_hdgm_duration_results.txt', 'w');

fprintf(fileID, 'UNIVARIATE HDGM RESULTS - DURATION\n');
fprintf(fileID, '==================================\n\n');
...
for i = 1:length(data.X_beta_name{1})
    fprintf(fileID, '%-12s: %8.4f\n', data.X_beta_name{1}{i}, duration_model.stem_par.beta(i));
end
fprintf(fileID, '\n');
