%% HDGM Model for Pickups and Duration
...
% Create the model
hdgm_model = stem_model(obj_stem_data, obj_stem_par);

% Set initial parameter values
obj_stem_par.beta = hdgm_model.get_beta0(); % Initial beta values
...
% Set initial values
hdgm_model.set_initial_values(obj_stem_par);

%% Model Estimation

% Data transform
hdgm_model.stem_data.log_transform; % Log-transform the data
hdgm_model.stem_data.standardize;   % Standardize the data
...
hdgm_model.EM_estimate(obj_stem_EM_options);

%% Model Validation

% Compute performance metrics
hdgm_model.set_varcov; % Set variance-covariance
hdgm_model.set_logL;   % Set log-likelihood

% Validation results
RMSE_pickups = sqrt(mean(hdgm_model.stem_validation_result{1, 1}.cv_mse_s)); % RMSE for pickups
RMSE_duration = sqrt(mean(hdgm_model.stem_validation_result{1, 2}.cv_mse_s)); % RMSE for duration
...
%% Save Results
save('hdgm_model_results.mat', 'hdgm_model'); 

% Access the beta coefficients
beta_selected = hdgm_model.stem_par.beta; % Beta coefficients for both Pickups and Duration

% Determine the number of covariates
num_covariates = size(beta_selected, 1) / 2; % Half for Pickups and half for Duration

% Separate the coefficients for Pickups and Duration
beta_pickups = beta_selected(1:num_covariates); % Coefficients for Pickups
beta_duration = beta_selected(num_covariates + 1:end); % Coefficients for Duration

% Get the names of the covariates
covariates_names = data.X_beta_name{1}; % Names of covariates

% Save results in .txt file
fileID = fopen('hdgm_model_results.txt', 'w');  % Open a text file for writing

% Write beta coefficients for Pickups
fprintf(fileID, 'Beta coefficients for Pickups:\n');
for i = 1:num_covariates
    fprintf(fileID, '%s: %.4f\n', covariates_names{i}, beta_pickups(i));
end
fprintf(fileID, '\n');
...
for i = 1:num_covariates
    fprintf(fileID, '%s: %.4f\n', covariates_names{i}, beta_duration(i));
end
fprintf(fileID, '\n');

% Write CV RMSE
fprintf(fileID, 'CV RMSE for Pickups: %.4f\n', RMSE_pickups);
fprintf(fileID, 'CV RMSE for Duration: %.4f\n', RMSE_duration);
fprintf(fileID, '\n');

% Write overall Log-Likelihood
fprintf(fileID, 'Overall Log-Likelihood: %.4f\n', logL);
fprintf(fileID, '\n');

% Close the file
fclose(fileID);
disp('hdgm_model_results.txt saved.');