%% Results Saving Script for Univariate Duration D-STEAM Model
clc;
warning off;

%% Load the results if not already loaded
if ~exist('duration_selected_model', 'var')
    load('duration_selected_model_results.mat');
end
...
results.beta = duration_selected_model.stem_par.beta;
...
results.RMSE = sqrt(mean(duration_selected_model.stem_validation_result{1,1}.cv_mse_s));

% 3. Extract Log-Likelihood
results.logL = duration_selected_model.stem_EM_result.logL;

% 4. Extract covariance parameters
results.v_p = duration_selected_model.stem_par.v_p;         % Spatial variance
results.theta_p = duration_selected_model.stem_par.theta_p; % Spatial range
results.sigma_eta = duration_selected_model.stem_par.sigma_eta; % Process variance
results.G = duration_selected_model.stem_par.G;             % Transition matrix
results.sigma_eps = duration_selected_model.stem_par.sigma_eps; % Error variance

%% Create and write to text file
filename = 'univariate_duration_model_results.txt';
fileID = fopen(filename, 'w');

% Write header
fprintf(fileID, 'UNIVARIATE D-STEAM MODEL RESULTS (DURATION)\n');
fprintf(fileID, '===========================================\n\n');
...
% 4. Write EM algorithm information
fprintf(fileID, '\nESTIMATION DETAILS:\n');
fprintf(fileID, '------------------\n');
if isfield(duration_selected_model.stem_EM_result, 'n_iter')
    fprintf(fileID, 'EM iterations      : %d\n', duration_selected_model.stem_EM_result.n_iter);
else
    fprintf(fileID, 'EM iterations      : Information not available\n');
end

if isfield(duration_selected_model.stem_EM_result, 'convergence')
    fprintf(fileID, 'Convergence reached: %d\n', duration_selected_model.stem_EM_result.convergence);
else
    fprintf(fileID, 'Convergence reached: Information not available\n');
end

if isfield(duration_selected_model.stem_EM_result, 'exit_tol')
    fprintf(fileID, 'Final tolerance    : %8.4e\n', duration_selected_model.stem_EM_result.exit_tol);
else
...
% Write to CSV
writetable(beta_table, 'univariate_duration_coefficients.csv');
disp('Beta coefficients also saved to univariate_duration_coefficients.csv');